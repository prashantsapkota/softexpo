...
function ServiceCategory() {
    ...
    const HandleAdd = (e) => {
        e.preventDefault();
        setadding(true);
        AdminAxios.post('/service_category/store', { name: NewCat }).then((res) => {
    ...
    const HandleDelete = (e, itemId) => {
        e.preventDefault()
        AdminAxios.delete(`/service_category/delete/${itemId}`).then(res => {
            if (res.status == 200) {
    ...
    const sendUpdateRequest = (e) =>{
        e.preventDefault()
        AdminAxios.put(`service_category/update/${editId}`,{name: edit_cat}).then(res=>{
            if (res.status == 200) {
                toast.info('🦄' + "Updated", {
                    position: "top-left",
                    autoClose: 5000,
