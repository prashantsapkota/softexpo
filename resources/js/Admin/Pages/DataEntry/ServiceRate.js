...
function Rate() {
    ...
    const HandleAdd = (e) => {
        e.preventDefault();
        setadding(true);
        AdminAxios.post('/hourly_rate/store', { rate: NewCat }).then((res) => {
    ...
    const HandleDelete = (e, itemId) => {
        e.preventDefault()
        AdminAxios.delete(`/hourly_rate/delete/${itemId}`).then(res => {
            if (res.status == 200) {
    ...
    const sendUpdateRequest = (e) =>{
        e.preventDefault()
        AdminAxios.put(`hourly_rate/update/${editId}`,{rate: edit_cat}).then(res=>{
            if (res.status == 200) {
                toast.info('🦄' + "Updated", {
                    position: "top-left",
                    autoClose: 5000,
                                ...
                                {Categories.map((c, i) => {
                                    return (<div key={i} className="d-flex my-2">
                                        {(editIndex === i) ?
                                            ...
                                            : <>
                                                <span className="flex-sm-fill">{c.rate}</span>
